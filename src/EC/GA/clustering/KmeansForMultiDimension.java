...
import EC.GA.*;
import java.io.*;
import EC.GA.clustering.KMeansClustering;;

public class KmeansForMultiDimension {
	...
	public static int Clustering(GAPopulation population, int setpoint, int generation) throws Exception
	{
...
//				isNumberOfClustersFound = false;
			if(Math.abs(Delta[k-2]/Delta[k-1])>6 && !isNumberOfClustersFound)
			{
				currentNumberOfClusters = k;
				isNumberOfClustersFound = true;
			}
		...
		if(isNumberOfClustersFound == false && currentNumberOfClusters == 1)
		{
			//for(int k=2; k<2*setpoint; k++)
			for(int k=2; k<10; k++)
			{
				if(Math.abs(Delta[k-2]/Delta[k-1])>5 && !isNumberOfClustersFound)
				{
					currentNumberOfClusters = k;
					isNumberOfClustersFound = true;
				}
		...
		if(isNumberOfClustersFound == false && currentNumberOfClusters == 1)
		{
			...
			//for(int k=2; k<2*setpoint; k++)
			{
				if(Math.abs(Delta[k-2]/Delta[k-1])>4 && !isNumberOfClustersFound)
				{
					currentNumberOfClusters = k;
					isNumberOfClustersFound = true;
				}
		...
		if(isNumberOfClustersFound == false && currentNumberOfClusters == 1)
		{
			...
			//for(int k=2; k<2*setpoint; k++)
			{
				if(Math.abs(Delta[k-2]/Delta[k-1])>3.2 && !isNumberOfClustersFound)
				{
					currentNumberOfClusters = k;
					isNumberOfClustersFound = true;
				}
		...
		/*while(Deviation[currentNumberOfClusters-1]<1 && currentNumberOfClusters>1)
	...
	public static  double performClustering(GAPopulation population, int K, boolean print)
	{
		...
		while(!isAllClustersDetermined)
		{
			...
			for(int k=0; k<K && numberOfChanges==0; k++)
				for(int i=0; i<cards[k] && numberOfChanges==0; i++)
				{
					...
					if(numberOfChanges==0||numberOfIterations>10000)
